...
public class StudyDashboardField {

    private final int totalNumberOfEvents;

    public StudyDashboardField(int totalNumberOfEvents) {
        this.totalNumberOfEvents = totalNumberOfEvents;
    }

    public static void main(String[] args) throws IOException, InterruptedException {
        StudyDashboardField studyDashboard = new StudyDashboardField(15);
    ...
    private double getRate(Participant p) {
        long count = p.homework().values().stream()
                .filter(v -> v == true)
                .count();
        return count * 100 / this.totalNumberOfEvents;
    }
